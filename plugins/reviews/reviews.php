...
//	cot_print('step');
	...
	cot_block(empty($item));
        ...
        $code = (!empty($code)) ? $code : cot_import('code', 'P', 'TXT');
        $query = "SELECT * FROM $db_reviews"
            ." WHERE item_touserid = $touser AND item_code = $code AND item_userid = " . $usr['id'];
        $sql = $db->query($query);
...
elseif ($a == 'update')
{
	...
	$item = $sql->fetch();
        ...
        $days = (int)$cfg['plugin']['reviews']['editdays'];
        $lastEditDay = $item['item_date'] + $days * 24 * 60 * 60 ;
        $now = $sys['now'];
        
        cot_die($lastEditDay < $now);
