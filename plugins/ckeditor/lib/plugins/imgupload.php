...
if(isset($_FILES['upload']) && strlen($_FILES['upload']['name']) > 1) {
  ...
  $img_name = base_convert(time(), 10, 36).'and'.base_convert(rand(0, 2000000000), 10, 36).'-'.basename($_FILES['upload']['name']);
  ...
  if(!in_array($type, $imgsets['type'])) $err .= ' Расширение файла '. $_FILES['upload']['name']. ' недопустимо. \\n Разрешается загружать файлы bmp, gif, jpg, jpe и png';
  if($_FILES['upload']['size'] > $imgsets['maxsize']*1000) $err .= '\\n Вес файла не должен превышать '. $imgsets['maxsize']. ' KB.';
  if(isset($width) && isset($height)) {
    if($width > $imgsets['maxwidth'] || $height > $imgsets['maxheight']) $err .= '\\n Ширина x Высота = '. $width .' x '. $height .' \\n Максимальное значение ширины и высоты не должно превышать: '. $imgsets['maxwidth']. ' x '. $imgsets['maxheight'];
    if($width < $imgsets['minwidth'] || $height < $imgsets['minheight']) $err .= '\\n Ширина x Высота = '. $width .' x '. $height .'\\n Значение ширины и высоты должно быть не менее: '. $imgsets['minwidth']. ' x '. $imgsets['minheight'];
  }
  ...
  if($err == '') {
    if(move_uploaded_file($_FILES['upload']['tmp_name'], $uploadpath)) {
      ...
      $message = 'Файл '. $img_name .' успешно загружен: \\n- Вес: '. number_format($_FILES['upload']['size']/1024, 3, '.', '') .' KB \\n- Размер: '. $width. ' x '. $height;
      $re = "window.parent.CKEDITOR.tools.callFunction($CKEditorFuncNum, '$url', '$message')";
    }
    else $re = 'alert("Файл не загрузился. Попробуйте загрузить еще раз.")';
  }
  else $re = 'alert("'. $err .'")';
}
