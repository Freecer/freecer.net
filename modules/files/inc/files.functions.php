...
function cot_files_get_someone_image($source, $item, $field = '', $field2 = '')
{
    if ( cot_files_count($source, $item, $field) > 0){
        return cot_files_get($source, $item, $field, 'path');
    }
    if( cot_files_count($source, $item, $field2) > 0 ) {
        if ( cot_files_count($source, $item, $field2, 'images') > 0 ){
            return cot_files_get($source, $item, $field2, 'path', 1, 1);
        }
        return cot_file_get_icon_by_id(
                cot_files_get($source, $item, $field2, 'id', 'first')
            );
    }
    
    return '/images/filesIcon/nov.png';
}
 ...
 */
function cot_files_get($source, $item, $field = '', $column = '', $number = 'first', $isImage = 0){
...
function cot_file_get_icon_by_id($item){
    if( (int)$item == 0 ){
        return '';
    }
    
    $file = files_model_File::getById($item);
...
function getFileConfParametr($key){
    return cot::$cfg['files'][$key];
}
