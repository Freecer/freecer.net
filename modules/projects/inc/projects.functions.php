...
function cot_projects_selectcat_multis($check, $name, $subcat = '', $hideprivate = true)
{
    global $structure;
    ...
    foreach ($structure['projects'] as $i => $x)
    {
       ...
       if(!empty($_GET["c"]) && is_array($_GET["c"])){
        ...
        foreach($result_array as $key => $val){
            $result .= '<label><input type="checkbox" name="c[]" value="'.$key.'" class="i-check" '. $active[$key].'></i> '.$val.'</label>';
        }
    return($result);
} 

if(!empty($_GET["saveFilter"]) && !empty($_GET["forpro"])){
    setcookie("forpro", 1);
}elseif(!empty($_GET["saveFilter"]) && empty($_GET["forpro"])){
    setcookie("forpro", 0);
}
 
if(!empty($_COOKIE["forpro"])){
    $_GET["forpro"] = $_COOKIE["forpro"];
}
if(!empty($_GET["saveFilter"]) && !empty($_GET["forpro"])){
    setcookie("forpro", 1);
}elseif(!empty($_GET["saveFilter"]) && empty($_GET["forpro"])){
...
function cot_projects_selectcat_multi($parent = '', $selected = array(), $level = 0, $template = '')
{
	...
	$t1 = new XTemplate(cot_tplfile(array('projects', 'treecheck', $template), 'module'));
	...
	$active = array();
       if(!empty($_POST["c"]) && is_array($_POST["c"])){
	...
	foreach ($children as $row)
	{
		$t1->assign(array(
			...
			"ROW_SELECTED" => $active[$row],
			"ROW_SUBCAT" => cot_projects_selectcat_multi($row, $selected, $level + 1, 'inner'),
			"ROW_LEVEL" => $level,
		));
