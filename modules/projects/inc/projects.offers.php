	...
	if ($usr['id'] == $offer['offer_userid'] || $usr['id'] == $item['item_userid'] || $usr['isadmin'])
	{
		...
		while ($posts = $sql_prjposts->fetch())
		{
			...
			$t_o->assign(array(
				...
				"POST_ROW_ID" => cot_parse($posts['post_id']),
			));
			/* === Hook === */
		foreach (cot_getextplugins('projects.posts.rows.tags') as $pl)
		{
			include $pl;
		}
		/* ===== */
		...
		/* === Hook === */
		foreach (cot_getextplugins('projects.posts.add') as $pl)
		{
			include $pl;
		}
		/* ===== */
