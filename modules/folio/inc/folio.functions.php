...
function cot_build_structure_folio_tree($parent = '', $selected = array(), $level = 0, $template = '')
{
	...
	foreach ($children as $row)
	{
		$jj++;
		$t1->assign(array(
			"ROW_ID" => $structure['folio'][$row]['id'],
			"ROW_PATH" => $structure['folio'][$row]['path'],
			"ROW_TPL" => $structure['folio'][$row]['tpl'],
...
function cot_getfoliolistWithCheckFiles($template = 'index', $count = 5, $sqlsearch = '',
							 $order = "item_date DESC")
{
	global $db, $db_folio, $cfg, $db_users;
